...
fisher.bintest <- function(formula, data, alpha = 0.05, p.method = "fdr") {
  ...
  result$method.test <- "Fisher's Exact"
  if (test$p.value < alpha & nlevels(fact) > 2) {
    ...
    result$method.multcomp <- "Fisher's exact"
  }
  class(result) <- "RVtest"
  return(test)
}
...
merge_cols <- function(x, y, by) {
  ...
  y_select_filter <- y_select_filter %>% dplyr::filter_all(dplyr::all_vars(!is.na(.)))
  # y_select <- y_select %>%
...
plot_multi_histogram <- function(df, feature, label_column) {
  ...
  plt <- ggplot(df, aes(x = eval(parse(text = feature)), fill = eval(parse(text = label_column)))) +
    geom_histogram(alpha = 0.3, position = "identity", binwidth = NULL, aes(y = ..density..),
...
boxplot_func_m <- function(df = ADSX, yvar = donation, treatvar = Treatment,
  comparisons = list(c("No ask-Domestic", "Domestic-Domestic"))) {
  yvar <- enquo(yvar)
  treatvar <- enquo(treatvar)
  ungroup(df) %>% group_by(!!treatvar) %>% drop_na(!!yvar,
    ...
    ggplot(aes(!!treatvar, !!yvar)) + geom_boxplot() +   theme(axis.title = element_text(size = 14),
    axis.text = element_text(size = 14)) + theme(axis.text.x = element_text(size = 12)) +
    labs(title = treatvar, y = yvar, caption = "p-values of Wilcox-(below) and  t-test (above brackets), means in red") +
    stat_summary(fun.y = mean, geom="point",colour="darkred", size=3) +
    geom_signif(comparisons = comparisons, step_increase = c(0.4), vjust = 1.7, margin_top = 0.5, textsize = 5) + geom_signif(comparisons = comparisons,
    step_increase = c(0.4), vjust = 0, margin_top = 0.5,
    textsize = 5, test = "t.test")
}
